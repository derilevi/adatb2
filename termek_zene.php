...
if(isset($_POST['add_termek'])) {	
	...
	*/
	...
	if(empty($cim) || empty($kiadas_eve) || empty($leiras) || empty($ar) || empty($kiado_id) || empty($kiado_id) || empty($e_mufajok_ids) || empty($e_hossz)) {
		...
		//Insert data
		$insert = "INSERT INTO $table_termekek_zenek(termek_id, idotartam) VALUES ($last_insert_ID, $hossz)";
...
if(edit_mode()) {
	...
	if(isset($_POST['termek_delete'])) {
		...
		$delete_result = Query("DELETE FROM $table_termekek WHERE termek_id=".$termek_id);
		
		$delete_result = Query("DELETE FROM $table_termekek_zenek WHERE termek_id=".$termek_id);
		
		
		header("Location: termek_zene.php");
	}
	...
	if(isset($_POST['termek_apply_edit'])) {
		...
		else
		{
			...
			//update
			$qry = "UPDATE $table_termekek_zenek SET idotartam=".$hossz." WHERE termek_id=".$termek_id;
...
if(isset($_GET['id']) && $_GET['id'] != null) {
	$termek_id = $_GET['id'];
	set_last_page("termek_zene.php?id=".$termek_id);
}
...
// ###########################
	...
	$result_zenek = Query("SELECT * FROM $table_termekek_zenek WHERE termek_id=".$termek_id);
	
	//if (mysql_num_rows($result_zenek) == 0)
	if($result_zenek == null) {
		echo "NO Termek in the Database";
		exit;
	}
	
	//$zene = mysql_fetch_assoc($result_zenek);
	$zenek = "";
	foreach($result_zenek as $zenek);
...
// ###########################
	...
	$result_termek = Query("SELECT * FROM $table_termekek_zenek");
	...
	foreach($result_termek as $termek) {
		...
		$result_zene = Query("SELECT * FROM $table_termekek WHERE termek_id=".$termek['TERMEK_ID']);
		
		$zene = "";
		foreach($result_zene as $zene);
		...
		$termekek .= CreateFormatRowFromZeneBasic($termek, $zene);
					...
					<article class="container" id="top">
						<div class="row">
							...
							<div class="8u 12u(mobile)">
								...
								<p>Hossz: '.$zenek['IDOTARTAM'].'</p>

								
								'. (edit_mode() ? '<a href="termek_zene.php?id='.$termek['TERMEK_ID'].'&termek_edit" class="button big scrolly">Szerkesztés</a>' : '' ).'
				...
				<article class="container" id="top">
					<div class="row">
						...
						<div class="8u 12u(mobile)">
							...
							<p>Hossz: 
							<input type="text" name="e_hossz" value="'.$zenek['IDOTARTAM'].'"></p>
		...
		<nav id="nav">
			<ul class="container">
				...
				<?php 
					...
					echo LinkGenerator('termek_zene.php');
					if(is_termek()) echo '<li><a href="termek_zene.php">Zene Kereső</a></li>'; 
