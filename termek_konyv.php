...
if(edit_mode()) {
	session_start();
	set_last_page("termek_konyv.php");
}
...
if(isset($_POST['add_termek'])) {	
	...
	*/
	...
	if(empty($cim) || empty($kiadas_eve) || empty($leiras) || empty($ar) || empty($kiado_id) || empty($kiado_id) || empty($e_mufajok_ids) || empty($e_hossz) 
		...
		//insert
		$insert = "INSERT INTO $table_termek_konyvek(termek_id, meret, oldalszam, kotes) VALUES ($last_insert_ID, $e_meret, $e_oldal, $e_kit)";
...
if(edit_mode()) {
	...
	if(isset($_POST['termek_delete'])) {
		...
		Query("DELETE FROM $table_termekek WHERE termek_id=".$termek_id);
		
		Query("DELETE FROM $table_termekek_konyvek WHERE termek_id=".$termek_id);
		
		
		header("Location: termek_konyv.php");
	}
	...
	if(isset($_POST['termek_apply_edit'])) {
		...
		else
		{
			...
			//update
			$qry = "UPDATE $table_termekek_konyvek SET kotes='".$e_kit."', oldalszam=".$e_oldal.", meret=".$e_meret." WHERE termek_id=".$termek_id;
...
if(isset($_GET['id']) && $_GET['id'] != null) {
	$termek_id = $_GET['id'];
	set_last_page("termek_konyv.php?id=".$termek_id);
}
...
// ###########################
	...
	$result_ekonyvek = Query("SELECT * FROM $table_termekek_konyvek WHERE termek_id=".$termek_id);
	...
	//$konyv = mysql_fetch_assoc($result_ekonyvek);
	$konyvek = "";
	foreach($result_ekonyvek as $konyvek);
...
// ###########################
	...
	$result_termek = Query("SELECT * FROM $table_termekek_konyvek");
	...
	foreach($result_termek as $termek) {
		...
		$result_konyv = Query("SELECT * FROM $table_termekek WHERE termek_id=".$termek['TERMEK_ID']);
		
		$konyv = "";
		foreach($result_konyv as $konyv);
		...
		$termekek .= CreateFormatRowFromKonyvBasic($termek, $konyv);
		...
		<article class="container" id="termek_add">
			<div class="row">
				...
				<form method="post">
					...
					Kötés:<input type="text" name="e_kit">
					...
					<article class="container" id="top">
						<div class="row">
							...
							<div class="8u 12u(mobile)">
								...
								<p>Méret: '.$konyvek['MERET'].'</p>
								
								<p>Oldalszám: '.$konyvek['OLDALSZAM'].'</p>
								
								<p>Kötés: '.$konyvek['KOTES'].'</p>
								
								
								

								
								'. (edit_mode() ? '<a href="termek_konyv.php?id='.$termek['TERMEK_ID'].'&termek_edit" class="button big scrolly">Szerkesztés</a>' : '' ).'
				...
				<article class="container" id="top">
					<div class="row">
						...
						<div class="8u 12u(mobile)">
							...
							<p>Méret: 
							<input type="text" name="e_meret" value="'.$konyvek['MERET'].'"></p>
							
							<p>Oldalszám: 
							<input type="text" name="e_oldal" value="'.$konyvek['OLDALSZAM'].'"></p>
							
							<p>Kötés: 
							<input type="text" name="e_kit" value="'.$konyvek['KOTES'].'"></p>
		...
		<nav id="nav">
			<ul class="container">
				...
				<?php 
					...
					echo LinkGenerator('termek_konyv.php');
					if(is_termek()) echo '<li><a href="termek_konyv.php">Könyv Kereső</a></li>'; 
