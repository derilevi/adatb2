...
{	
	...
	*/
	...
	//IF any Data missing
	if(empty($cim) || empty($kiadas_eve) || empty($leiras) || empty($ar) || empty($kiado_id) || empty($kiado_id) || empty($e_mufajok_ids)) {
		//Should drop error
...
if(isset($_GET['id']) && $_GET['id'] != null) {
	$termek_id = $_GET['id'];
	set_last_page("termek.php?id=".$termek_id);
}
...
// ###########################
	...
	//if (mysql_num_rows($result_termek) == 0)
	if($result_termek == null) {
		echo "NO Termek in the Database";
		exit;
	}
	
	//$film = mysql_fetch_assoc($result_termek);
	$termek = "";
						...
						<article class="container" id="top">
							<div class="row">
								...
								<div class="8u 12u(mobile)">
									...
									'. (edit_mode() ? '<a href="termek.php?id='.$termek['TERMEK_ID'].'&termek_edit" class="button big scrolly">Szerkesztés</a>' : '' ).'
		...
		<nav id="nav">
			<ul class="container">
				...
				<?php 
					...
					echo LinkGenerator('termek.php');
					if(is_termek()) echo '<li><a href="termek.php">Termék Kereső</a></li>'; 
